...
cursor.execute("""
CREATE TRIGGER IF NOT EXISTS instructor_username BEFORE INSERT ON Instructor
	FOR EACH ROW
		IF NEW.username in (
			select S.username
            from Student S
...
cursor.execute("""
CREATE TRIGGER IF NOT EXISTS student_username BEFORE INSERT ON Student
	FOR EACH ROW
		IF NEW.username in (
			select I.username
            from Instructor I
...
cursor.execute("""
CREATE TRIGGER IF NOT EXISTS same_department_id BEFORE INSERT ON COURSE
	FOR EACH ROW
		IF NEW.departmentID != (
...
cursor.execute("""
CREATE TRIGGER IF NOT EXISTS n_of_dbmanagers BEFORE INSERT ON DatabaseManager
	FOR EACH ROW
		IF (SELECT COUNT(*)
...
cursor.execute("""
CREATE TRIGGER IF NOT EXISTS quota_restriction BEFORE INSERT ON Enrolled
	FOR EACH ROW
		IF (SELECT COUNT(*)
...
cursor.execute("""
CREATE PROCEDURE IF NOT EXISTS FilterCourses(IN department_id int, IN campus_ varchar(255), IN min_credits int, IN max_credits int)
BEGIN
...
cursor.execute("""
CREATE PROCEDURE IF NOT EXISTS CreateInstructor(IN username varchar(255), IN title varchar(255), IN password varchar(255), IN name varchar(255), IN surname	varchar(255), IN email varchar(255), IN departmentID int)
BEGIN
...
cursor.execute("""
CREATE PROCEDURE IF NOT EXISTS CreateStudent(IN studentID int, IN username varchar(255), IN password varchar(255), IN name varchar(255), IN surname varchar(255), IN email varchar(255), IN departmentID int)
BEGIN
INSERT INTO Student VALUES (studentID, 0, 0, username, password, name, surname, email, departmentID);
END;
""")

cursor.execute("""
CREATE PROCEDURE IF NOT EXISTS CreateCourse(IN courseID int, IN name varchar(255), IN departmentID int, IN credits int, IN instructor_username varchar(255), IN classroomID int, IN quota int)
BEGIN
